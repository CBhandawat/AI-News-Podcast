...
def main(args):
    ...
    if not fast_check_ffmpeg():
        logger.warning("Adding ffmpeg to PATH")
    ...
    for task_id in inference_config:
        try:
            ...
            # Sanitize paths
            video_path = video_path.encode('ascii', 'ignore').decode('ascii')
            audio_path = audio_path.encode('ascii', 'ignore').decode('ascii')
            if args.output_vid_name:
                args.output_vid_name = args.output_vid_name.encode('ascii', 'ignore').decode('ascii')
            logger.info(f"Processing task: video_path={video_path}, audio_path={audio_path}, output_vid_name={args.output_vid_name}")
            ...
            crop_coord_save_path = os.path.join(args.result_dir, "../", f"{input_basename}.pkl")
            ...
            if os.path.exists(crop_coord_save_path) and args.use_saved_coord:
                logger.info("Using saved coordinates")
                with open(crop_coord_save_path, 'rb') as f:
                    coord_list = pickle.load(f)
                frame_list = read_imgs(input_img_list)
            else:
                logger.info("Extracting landmarks... time-consuming operation")
                coord_list, frame_list = get_landmark_and_bbox(input_img_list, bbox_shift)
                with open(crop_coord_save_path, 'wb') as f:
                    pickle.dump(coord_list, f)
            
            logger.info(f"Number of frames: {len(frame_list)}")         
            ...
            logger.info(f"Video generation command: {cmd_img2video}")
            os.system(cmd_img2video)   
            
            cmd_combine_audio = f"ffmpeg -y -v warning -i {audio_path} -i {temp_vid_path} {output_vid_name}"
            logger.info(f"Audio combination command: {cmd_combine_audio}") 
            ...
            logger.info(f"Results saved to {output_vid_name}")
